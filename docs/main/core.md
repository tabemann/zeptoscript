...
Saved States represent a context to return to, i.e. the state of the data and return stacks. They are gotten through passing an execution token to `save`, which will be passed the saved state of that call to `save`, and they may be returned to through calling the saved state with a single argument, which will be placed on the data stack after the saved state is returned from.
...
Call *xt* with the saved state of the call to `save`. When the saved state is called, the the context of the original call to `save` will be returned to, with the top element of the data stack being preserved.
